define(['underscore', 'backbone', 'jquery', 'lib/reefbookjs/jquery-event-destroyed'], function (_, Backbone, $) {
	var ReefBookJS = {};
	ReefBookJS.VERSION = '2.1.2';
	// Avoid GET caching issues with Internet Explorer
	if (XMLHttpRequest) {
		$.ajaxSetup({cache: false});
	}
	ReefBookJS.Validation = (function () {
		var ReefBookJSValidation = {};
		...
		ReefBookJSValidation.options.URL = {
		...
		ReefBookJSValidation.addValidator = function (constraintType, validator) {
			validators[constraintType] = validator;
		};
		// retrieve the validator associated to a given constraint type
		ReefBookJSValidation.getValidator = function (constraintType) {
		...
		var nullValidator = function (value, msg) {
			if (ReefBookJSValidation._hasValue(value)) {
				return msg;
			}
		};
		...
		var assertTrueValidator = function (value, msg) {
			if (ReefBookJSValidation._hasValue(value) && ((_.isString(value) && value.toLowerCase() != "true") || !value)) {
				return msg;
			}
		};
		var assertFalseValidator = function (value, msg) {
			if (ReefBookJSValidation._hasValue(value) && ((_.isString(value) && value.toLowerCase() == "true") || (_.isBoolean(value) && value))) {
				return msg;
			}
		};
		var minValidator = function (value, min, msg) {
			var numValue = parseInt(value);
			if (ReefBookJSValidation._hasValue(value) && (isNaN(numValue) || (numValue != value) || (numValue < min))) {
				return msg;
			}
		};
		var maxValidator = function (value, max, msg) {
			var numValue = parseInt(value);
			if (ReefBookJSValidation._hasValue(value) && (isNaN(numValue) || (numValue != value) || (numValue > max))) {
				return msg;
			}
		};
		...
		var urlValidator = function (value, protocol, host, port, regexp, msg) {
			if (!_.isString(value) || !value.match(ReefBookJSValidation.options.URL.urlPattern)) {
				return msg;
			}
			if (regexp && !value.match(regexp)) {
				return msg;
			}
			var urlParts = value.match(ReefBookJSValidation.options.URL.urlParser);
		...
		ReefBookJSValidation.forceSynchroForClass = function (className) {
		...
		ReefBookJSValidation._hasValue = function (value) {
		...
		ReefBookJSValidation._trim = String.prototype.trim ?
			function (text) {
		...
		ReefBookJSValidation.synchronize = function (model, errorCallback, successCallback) {
			...
			if (!synchronizedClasses[model.prototype.className]) {
				...
				$.getJSON(ReefBookJSValidation.options.apiUrl + '/' + model.prototype.className, {locale: locale})
					.success(_.bind(function (resp) {
						buildValidation(resp, model, _.extend(msgs, ReefBookJSValidation.messages, model.prototype.messages));
					...
					.error(function (resp) {
						...
						else ReefBookJSValidation.options.errorCallback(resp);
					});
			}
		};
		ReefBookJSValidation.options.errorCallback = function (resp) {
			console.error("error calling server : status code " + resp.status + " and text '" + resp.statusText + "'");
		};
		return ReefBookJSValidation;
	})();
	// extend **Backbone.View** properties and methods.
	ReefBookJS.View = Backbone.View.extend({
		ReefBookJSViewOptions: ['model', 'collection', 'el', 'id', 'attributes', 'className', 'tagName', 'root', 'strategy', 'context'],
		...
		_configure: function (options) {
			...
			for (var i = 0, l = this.reefbookjsViewOptions.length; i < l; i++) {
				var attr = this.reefbookjsViewOptions[i];
		...
		delegateEvents: function (events) {
			ReefBookJS.View.__super__.delegateEvents.apply(this, arguments);
		...
		setElement: function (element, delegate) {
			ReefBookJS.View.__super__.setElement.apply(this, arguments);
			if (this.root) {
				this._ensureRoot();
				this._insertRoot();
			}
		...
		remove: function () {
			...
			ReefBookJS.View.__super__.remove.apply(this, arguments);
			return this;
		},
